...
.footnote[Note: this is the same equation as Marginal Revenue for a monopoly! Just replace $p$ with $w$ and $Q$ with $L$.]

---

# Monopsony and Marginal Cost of Labor II

- If we have a linear inverse supply function of the form
$$w=a+bL$$
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  stat_function(fun=supply, geom="line", size=1, color="red")+
  geom_label(x=7.5,y=supply(7.5), label="Supply (slope=b)", color="red")+
  stat_function(fun=MC_mon, geom="line", size=1, color="darkred")+
  geom_label(x=3.75,y=MC_mon(3.75), label="MC(L) (slope=2b)", color="darkred")+
    scale_x_continuous(breaks = c(0),
                       labels = c(0),
                       expand=expand_scale(mult=c(0,0.1)))+
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  stat_function(fun=MC_mon, geom="line", color="darkred", size=1)+
