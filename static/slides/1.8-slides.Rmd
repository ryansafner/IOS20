...
- Recall .hi-red[market supply] is the **minimum willingness to accept**, the minimum price necessary to bring a resource to market
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  stat_function(fun=supply_firm, geom="line", size=2, color = "red")+
    ...
    geom_segment(aes(x=2.5,y=0),xend=2.5,yend=5,linetype=3)+
      geom_segment(aes(x=1.25,y=0),xend=1.25,yend=5,linetype=3)+
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  geom_label(x=4,y=mc_i(4), color = "darkred", label="MC(L)")+
    stat_function(fun=demand_ex, geom="line", color="blue", size=1)+
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  annotate("segment", x = 3.325, xend = 3.325, y = 12, yend = 11.625, colour = "purple", size=2, alpha=1, arrow=arrow(length=unit(0.5,"cm"), ends="last", type="closed"))+
