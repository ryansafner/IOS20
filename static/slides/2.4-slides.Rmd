---
title: "2.4: Sequential Games & Contestable Markets"
...
- See some visualizations I put on []
...
- We've already seen examples of continuous strategies in Cournot,  Bertrand, & Stackelberg competition
...
- Important aspects of Nash Equilibrium (N.E.):
  - N.E. $\neq$ the "best" pr optimal outcome
    - Recall the Prisoners' Dilemma!
  - A game may have **multiple** N.E.
  - A game may have **no** N.E. (in "pure" strategies)
...
- Each player faces at least one .hi-purple["decision node"] (solid, colored by player)
  ...
  - Game ends at any .hi-purple["terminal node"] (hollow node), and each player earns payoffs (.red[red to Entrant], .blue[blue to Incumbent])
...
- We have the outcome: (.hi-red[Enter], .hi-blue[Accommodate])
...
- Note, if .hi-red[Entrant] plays .hi-red[Stay Out], doesn't matter what .hi-blue[Incumbent] plays
...
1. **Cell-by-Cell Inspection**: look in each cell, does either player want to deviate?
  - If Yes: a Nash Equilibrium
  - If No: not a Nash Equilibrium
...
2. **Best-Responses**: take the perspective of each player. If the *other* player plays a particular strategy, what is *your* strategy that gets you the highest payoff?
  - Ties are allowed.
...
- That is a .hi-purple["subgame perfect Nash equilibrium"]
...
- It's never rational for .hi-blue[Incumbent] to play .blue[Fight] if the game reaches .blue[I.1]!
...
- Thus, Nash Equilibrium (.hi-red[Stay Out], .hi-blue[Fight]) is **not sequentially rationall**
...
- These strategies for each player constitute a Nash equilibrium in every possible subgame!
...
- This can happen with .hi[credible commitment]
]
...
.pull-left[
.center[
![:scale 70%](https://www.dropbox.com/s/hfzmi9modl88nfn/cableconstruction.jpg?raw=1)

![:scale 70%](https://www.dropbox.com/s/z3q98vngkautezz/cableinfrastructure.jpg?raw=1)
...
- This set of strategies induces a Nash equilibrium in every subgame
  - Each player optimally responding to every contigency

- This is the .hi[subgame perfect Nash equilibrium]
...
- Price competition between the two firms with similar products<sup>.hi-red[1]</sup> $\implies$ consumers buy only from the firm with the lower price
...
.footnote[<sup>.hi-red[1]</sup> In the canonical economic models of oligopoly, 
...
- **Nash equilibrium**: .hi-red[incumbent] sets $\color{red}{p_I}=\color{blue}{p_E}-\epsilon$
...
- **Nash equilibrium**: .hi-red[Incumbent] prices at $\color{red}{p_I}=AC$ earns $\pi=0$
...
- Nash equilibrium:
  - .hi-red[Incumbent] can deter entry with $\color{red}{p_I}=\color{blue}{p_E}-\epsilon$
